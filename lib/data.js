...
import { PRIORITY_BONUS } from './score';
...
/**
 * Единый порядок провайдеров для ВСЕХ списков-рейтингов на сайте (главная,
 * каталог, провайдеры, задачи, гео). Источник приоритета один — PRIORITY_BONUS
 * из lib/score. Сначала приоритетные по убыванию веса, затем остальные по
 * минимальной цене. Так наши провайдеры стоят наверху одинаково везде.
 */
export const providerPriority = (slug) => PRIORITY_BONUS[slug] || 0;
export function sortProvidersByPriority(list) {
  return [...list].sort(
    (a, b) =>
      // Timeweb Cloud всегда первый, где он вообще есть в списке
      (b.slug === 'timeweb') - (a.slug === 'timeweb') ||
      providerPriority(b.slug) - providerPriority(a.slug) ||
...
export function providersForTask(taskSlug) {
  return sortProvidersByPriority(PROVIDERS.filter((p) => (p.tasks || []).includes(taskSlug)));
}

export function providersForGeo(geoCode) {
  // Провайдер попадает в регион только при наличии проверенного тарифа именно с
  // этой локацией, а не просто заявленного региона в профиле.
  const withGeo = new Set(VISIBLE_PLANS.filter((p) => p.geo === geoCode).map((p) => p.providerSlug));
  return sortProvidersByPriority(PROVIDERS.filter((p) => withGeo.has(p.slug)));
}
