...
/**
 * Провайдеры, которые владелец держит в приоритете: их свежий материал
 * всегда стоит наверху ленты, даже если после него вышли другие публикации.
 * Наверх поднимается ТОЛЬКО последний материал каждого из них, по одному,
 * иначе десяток статей про Timeweb вытеснит с первого экрана всё остальное
 */
export const PRIORITY_PROVIDERS = ['timeweb', 'adminvps', 'hostman'];

/**
 * Порядок ленты: сверху свежий материал каждого приоритетного провайдера,
 * ниже всё остальное по дате. Даты на сайте не показываем, поэтому свежесть
 * читается меткой «Новое» — её получает самый новый материал по дате
 */
function ordered(posts) {
  const pinned = [];
  for (const slug of PRIORITY_PROVIDERS) {
    const latest = posts.find((p) => p.provider === slug);
    if (latest) pinned.push(latest.slug);
  }
  const newest = posts[0]?.slug;
  const mark = (p) => ({ ...p, pinned: pinned.includes(p.slug), fresh: p.slug === newest });
  return [
    ...posts.filter((p) => pinned.includes(p.slug)).map(mark),
    ...posts.filter((p) => !pinned.includes(p.slug)).map(mark),
  ];
}

export function allPosts() {
  return ordered(readAll());
}
...
function markExternalLinks(html) {
  // Свой домен берём из конфига, чтобы при смене адреса ссылка на самих себя
  // не начала считаться внешней. Абсолютных ссылок на себя в статьях нет
  // (используем относительные /novosti/...), но проверка должна пережить переезд
  return html.replace(/<a href="(https?:\/\/[^"]+)"/g, (m, href) =>
    href.includes(SITE.domain)
      ? m
...
export function getPost(slug) {
  // берём из allPosts, а не из readAll: иначе на странице материала
  // не будет признаков pinned/fresh и метка «Новое» не отрисуется
