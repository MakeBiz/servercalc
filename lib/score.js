...
export function runMatch(payload, query) {
  ...
  // Рекомендованным партнёрам добавляем открытую прибавку к баллу (не жёсткое
  // закрепление): позиция всегда следует за баллом, поэтому показанный процент
  // согласован с порядком. Провайдеры без приоритета считаются как раньше; у
  // приоритетных ставим флаг priority для бейджа «рекомендуем».
