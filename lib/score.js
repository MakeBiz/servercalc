...
/**
 * Рекомендованные партнёры, которые закрепляются выше в выдаче подбора.
 * Это редакционное закрепление порядка показа, а НЕ прибавка к проценту
 * соответствия: балл считается той же формулой без учёта комиссии.
 */
// Приоритет закрепления в выдаче (1 = верх). Порядок списка = порядок показа.
// beget в каталоге servercalc.ru отсутствует и просто игнорируется (список единый
// со вторым сайтом, чтобы приоритет был один на всё).
export const PINNED = ['ishosting', 'timeweb', 'adminvps', 'hostman', 'beget'];
/** Минимальная релевантность, при которой закрепление срабатывает */
export const PIN_FIT = 50;
...
export function runMatch(payload, query) {
  ...
  // Рекомендованные партнёры закрепляются выше в выдаче, но ТОЛЬКО когда реально
  // подходят под запрос (score >= PIN_FIT): нерелевантного провайдера наверх не тащим.
  // Процент соответствия при этом считается без учёта комиссии (см. scoreProvider),
  // меняется только порядок показа, и он открыто помечается бейджем «рекомендуем».
  const pinRank = (r) => {
    const i = PINNED.indexOf(r.provider.slug);
    return i === -1 || r.score < PIN_FIT ? Infinity : i;
  };
  return scored
    .map((r) => ({ ...r, pinned: pinRank(r) !== Infinity }))
    .sort((a, b) => {
      const pa = pinRank(a);
      const pb = pinRank(b);
      if (pa !== pb) return pa - pb;
      return b.score - a.score || a.plan.priceRub - b.plan.priceRub;
    });
}
