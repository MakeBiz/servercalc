...
function pickPlan(plans, query, geoCode) {
  ...
  // Если запрошена конкретная география и тарифа в ней нет — НЕ подставляем тариф
  // из другой страны: провайдер без сервера в нужной локации просто не участвует
  // в подборе по этой географии (претензия внешнего аудита про гео-фильтр).
  const pool = inGeo.length ? inGeo : geoCode ? [] : plans;
...
export function scoreProvider(provider, plans, query, dict = {}) {
  ...
  if (plan && dict.medianPerGb && plan.ram > 0) {
    ...
    // Балл за цену за ресурс учитываем всегда, но негативную формулировку
    // «цена выше медианы» на карточке не показываем (по решению владельца):
    // остаются только «на уровне медианы» и «ниже медианы».
    if (delta >= 0 || diff < 5) {
      reasons.push({
        code: 'value',
        ok: delta >= 0,
...
/**
 * Прибавка к проценту соответствия рекомендованным партнёрам. 1 = высший
 * приоритет = больше прибавка. Это редакционный приоритет показа, применяется
 * ПОВЕРХ открытой формулы; позиция всегда следует за баллом. Провайдеры без
 * приоритета считаются как раньше. Приоритет меняется одним этим объектом.
 * Значения подстраиваемые. Слаги, которых нет в каталоге (например cloudways),
 * просто игнорируются — список держим единым со вторым сайтом.
 */
...
export function runMatch(payload, query) {
  ...
  // Рекомендованным партнёрам добавляем прибавку к баллу; у приоритетных ставим
  // флаг priority для бейджа «рекомендуем». Timeweb Cloud дополнительно
  // закреплён на первом месте выдачи (см. ниже), поэтому его показанный балл
  // поднимается не ниже лидера, чтобы порядок и процент на карточке совпадали.
  const scored = payload.providers
    .map((provider) => scoreProvider(provider, byProvider.get(provider.slug) || [], query, dict))
    .filter((r) => r.plan)
    .map((r) => {
      const b = PRIORITY_BONUS[r.provider.slug] || 0;
      return b ? { ...r, score: clamp(r.score + b), priority: true } : r;
    })
    .sort((a, b) => b.score - a.score || a.plan.priceRub - b.plan.priceRub);

  // Жёсткое закрепление Timeweb Cloud на первом месте, если он вообще прошёл в
  // подбор (есть подходящий тариф под запрос). Балл поднимаем до уровня лидера,
  // чтобы карточка не показывала меньший процент выше большего.
  const twIdx = scored.findIndex((r) => r.provider.slug === 'timeweb');
  if (twIdx > 0) {
    const [tw] = scored.splice(twIdx, 1);
    tw.score = Math.max(tw.score, scored[0].score);
    tw.priority = true;
    scored.unshift(tw);
  }
  return scored;
